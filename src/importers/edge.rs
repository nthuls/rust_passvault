...
// Credential structure from Edge database (same as Chrome)
...
impl EdgeImporter {
    ...
    // Find Edge profiles on the system
        ...
        match self.os_type.as_str() {
            "windows" => {
                if let Some(local_app_data) = env::var_os("LOCALAPPDATA") {
                    Some(PathBuf::from(local_app_data).join("Microsoft\\Edge\\User Data"))
                } else {
                    None
                }
            },
            "macos" => {
                if let Some(home) = env::var_os("HOME") {
                    Some(PathBuf::from(home).join("Library/Application Support/Microsoft Edge"))
                } else {
                    None
                }
            },
            "linux" => {
                if let Some(home) = env::var_os("HOME") {
                    Some(PathBuf::from(home).join(".config/microsoft-edge"))
                } else {
                    None
                }
            },
    ...
    // Extract credentials from an Edge profile
    pub fn extract_credentials(&self, profile_path: PathBuf) -> Result<Vec<EdgeCredential>, EdgeImportError> {
        let login_data_path = profile_path.join("Login Data");
        
        if !login_data_path.exists() {
            return Err(EdgeImportError::ProfileNotFound(format!(
                "Login Data not found at: {}", login_data_path.display()
            )));
        }
        
        // Create a temporary copy of the database file because Edge might have it locked
        let temp_dir = tempfile::tempdir()?;
        let temp_db_path = temp_dir.path().join("temp_edge_login_data.db");
    ...
    // Decrypt Edge passwords (different methods depending on OS)
    // Edge uses the same encryption as Chrome, so we can reuse the same logic
    pub fn decrypt_credentials(&self, credentials: Vec<EdgeCredential>) -> Vec<(String, String, String)> {
        ...
        for cred in credentials {
            let decrypted_password = match self.os_type.as_str() {
                ...
                _ => Err(EdgeImportError::UnsupportedOS),
            };
    ...
    fn decrypt_password_windows(&self, encrypted: &[u8]) -> Result<String, EdgeImportError> {
        // Edge on Windows uses Windows Data Protection API (DPAPI)
        // The first 5 bytes are "DPAPI" prefix (v10) or 3 bytes (v1-v9)
        if encrypted.len() < 5 {
            return Err(EdgeImportError::DecryptionError("Encrypted data too short".to_string()));
        }
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if result == 0 {
                return Err(EdgeImportError::DecryptionError(
        ...
        {
            Err(EdgeImportError::UnsupportedOS)
        }
    }
    
    // macOS-specific decryption
    fn decrypt_password_macos(&self, encrypted: &[u8]) -> Result<String, EdgeImportError> {
        // Edge on macOS uses the system keychain
        // Similar to Chrome's implementation
        
        // Create a temporary file with the encrypted data
        let temp_dir = tempfile::tempdir()?;
        let temp_file_path = temp_dir.path().join("edge_password.bin");
        ...
        if !output.status.success() {
            // Try the Chrome key as fallback
            let chrome_output = Command::new("security")
                .args(&[
                    "find-generic-password",
                    "-a", "Chrome",
            ...
            if !chrome_output.status.success() {
                return Err(EdgeImportError::DecryptionError(
                    "Failed to retrieve Edge Safe Storage password".to_string()
                ));
            }
        }
        
        // Edge uses AES-128-CBC for encryption on macOS
        // This would require additional crypto code to implement
        // For now, return a placeholder error
        Err(EdgeImportError::DecryptionError(
            "macOS decryption not fully implemented".to_string()
        ))
    }
    
    // Linux-specific decryption
    fn decrypt_password_linux(&self, encrypted: &[u8]) -> Result<String, EdgeImportError> {
        ...
        // On Linux, Edge sometimes uses a hardcoded key or the GNOME keyring
        // Edge is based on Chromium, so it likely uses the same approach as Chrome
        
        // Try the hardcoded key first (same as Chrome)
        let key = b"peanuts";
        
        // Edge uses AES-128-CBC for encryption on Linux
        // This would require additional crypto code to implement
        // For now, return a placeholder error
        Err(EdgeImportError::DecryptionError(
    ...
    pub async fn import_passwords(
        ...
        let profile = if let Some(path) = profile_path {
            ...
            if profiles.is_empty() {
                return Err(EdgeImportError::ProfileNotFound(
                    "No Edge profiles found".to_string()
                ));
            }
            profiles[0].1.clone()
        };
